...
class BunkerSource<T: NSManagedObject>: NSObject, ObservableObject, NSFetchedResultsControllerDelegate {
    ...
    private let contextDidSaveNotifications = BunkerNotifier()
    ...
    public func sortKey2(_ sortKey2: String?) -> BunkerSource {
        self.sortKey2 = sortKey2
        return self
    }
    
    public func sectionNameKeyPath(_ sectionNameKeyPath: String?) -> BunkerSource {
        self.sectionNameKeyPath = sectionNameKeyPath
        return self
    }
    
    public func predicateKey(_ predicateKey: String?) -> BunkerSource {
        self.predicateKey = predicateKey
        return self
    }
    
    public func predicateObject(_ predicateObject: NSManagedObject?) -> BunkerSource {
        self.predicateObject = predicateObject
        return self
    }
    
    public func sortAscending1(_ sortAscending1: Bool?) -> BunkerSource {
        self.sortAscending1 = sortAscending1 ?? true
        return self
    }
    
    public func sortAscending2(_ sortAscending2: Bool?) -> BunkerSource {
        self.sortAscending2 = sortAscending2 ?? true
        return self
    }
    
    public func predicate(_ predicate: NSPredicate?) -> BunkerSource {
        self.predicate = predicate
        return self
    }
    
    public func entity(_ entity: NSEntityDescription?) -> BunkerSource {
        self.entity = entity
        return self
    }
    ...
    private func configureFetchedResultsController() -> NSFetchedResultsController<T> {
        ...
        let frc = NSFetchedResultsController(fetchRequest: fetchRequest,
                                             managedObjectContext: Bunker.stack.context,
    ...
    public func fetch() -> [T] {
        let fetchRequest = self.configureFetchRequest()
        do {
            let objects = try Bunker.stack.context.fetch(fetchRequest)
            return objects
    ...
    public func delete(from source: IndexSet) {
        Bunker.executeBlockAndCommit {
            for index in source {
                Bunker.stack.context.delete(self.fetchedObjects[index])
            }
        }
    }
    ...
    public func delete(from source: IndexSet, inSection: NSFetchedResultsSectionInfo) {
        Bunker.executeBlockAndCommit {
            for index in source {
                Bunker.stack.context.delete(self.objects(inSection: inSection)[index])
            }
        }
    }
